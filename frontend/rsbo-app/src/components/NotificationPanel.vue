...
export default {
  ...
  watch: {
    notifications: {
      handler() {
        // Emitimos la cantidad de no leídas cada vez que cambien las notificaciones
    ...
    // NOTA: Quitamos el watcher de 'visible' para que no marque todo como leído automáticamente
  },
  computed: {
    unreadNotifications() {
  ...
  methods: {
    // Quitamos markAllAsRead para que no marque todo al abrir el panel

    markAsRead(index) {
      const notif = this.notifications[index];
      if (!notif.read) {
        notif.read = true;
        notif.readAt = new Date(); // Guarda fecha actual

        // animación del ícono al marcar como leído
        this.$nextTick(() => {
          const icons = this.$el.querySelectorAll('.notification-item')[index].querySelectorAll('.b-icon');
          const icon = icons[1]; // segundo ícono = 'read'
          if (icon) {
    ...
    deleteNotification(index) {
      const notif = this.notifications[index];
      notif.tooltipActive = false; // 🔴 Cierra tooltip inmediatamente
      ...
      setTimeout(() => {
        ...
        if (notifEl) {
          ...
          }, 300); // ⏱️ Espera a que termine la animación antes de eliminar
        }
      }, 50); // ⏱️ Espera breve para dejar que desaparezca el tooltip visualmente
    }
    ...
    pinNotification(index) {
      ...
      // 🔴 1. Cerrar tooltip inmediatamente
      notif.tooltipActive = false;

      // 2. Esperar a que desaparezca el tooltip antes de mover el componente
      setTimeout(() => {
        // Reordenar lista
        if (!notif.pinned) {
        ...
        this.$nextTick(() => {
          ...
          // 🟢 3. Volver a activar el tooltip después de que termine la animación
          setTimeout(() => {
            notif.tooltipActive = true;
          }, 300);
        });
      }, 50); // ⏱️ Espera breve antes de empezar la animación
