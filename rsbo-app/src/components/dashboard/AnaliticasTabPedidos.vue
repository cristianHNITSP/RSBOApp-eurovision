  ...
  <div class="an-tab-two-col">
      ...
      <div class="gcard mb-5">
        ...
        <div class="gc-body">
          <div class="an-stat-grid">
            ...
            <div class="an-stat-cell">
              <div class="asc-ico" style="background:var(--c-primary-alpha);color:var(--c-primary)"><i
    ...
    <div class="an-col-side">
        ...
        <div class="gc-body">
          <template v-if="!isLoading">
            <div class="sl-display">
              <div class="sl-ring" :class="serviceLevelClass"><span class="sl-pct">{{ s?.serviceLevel ?? 0
              }}%</span><span class="sl-label-sm">{{ serviceLevelStatus }}</span></div>
              <div class="sl-details">
                <div class="sl-row"><span class="sl-k">Pedidos cerrados</span><b class="sl-v">{{ s?.ordersClosed30d ?? 0
                }}</b></div>
                <div class="sl-row"><span class="sl-k">Con corrección</span><b class="sl-v">{{ s?.corrections30d ?? 0
                }}</b></div>
                <div class="sl-row"><span class="sl-k">Tasa corrección</span><b class="sl-v">{{ correctionRate }}%</b>
                </div>
                <div class="sl-row"><span class="sl-k">Período</span><b class="sl-v">{{ s?.periodLabel || '30d' }}</b>
                </div>
              </div>
            </div>
            <b-progress :value="s?.serviceLevel ?? 0" size="is-small" :type="serviceLevelTagType" :show-value="false"
              class="mt-3" />
            <div class="sl-chip" :class="`sl-chip-${serviceLevelClass}`"><i class="fas fa-circle-info"></i> {{
              serviceLevelStatus }} — {{ serviceLevelComment }}</div>
          </template>
          <template v-else><b-skeleton width="100%" :height="92" animated class="mb-2" /><b-skeleton width="100%"
              :height="8" animated /></template>
        </div>
      </div>
      ...
      <div v-if="isRoot" class="gcard mb-4">
        ...
        <div class="gc-body">
          <div class="cov-row"><span class="cov-k">Hist. cerrados</span><b class="cov-v" v-if="!isLoading">{{
            fmtn(s?.ordersClosedAll) }}</b><b-skeleton v-else :width="55" :height="16" animated /></div>
          <div class="cov-row"><span class="cov-k">Total cancelados</span><b class="cov-v" v-if="!isLoading">{{
            s?.ordersCancelledAll ?? 0 }}</b><b-skeleton v-else :width="45" :height="16" animated /></div>
          <div class="cov-row"><span class="cov-k">Ediciones (30d)</span><b class="cov-v" v-if="!isLoading">{{
            s?.edits30d
            ?? 0 }}</b><b-skeleton v-else :width="45" :height="16" animated /></div>
          <div class="cov-row"><span class="cov-k">Movimientos hoy</span><b class="cov-v" v-if="!isLoading">{{
            s?.movementsToday ?? 0 }}</b><b-skeleton v-else :width="45" :height="16" animated /></div>
          <div class="cov-row"><span class="cov-k">Movimientos (30d)</span><b class="cov-v" v-if="!isLoading">{{
            fmtn(s?.movementsTotal30d) }}</b><b-skeleton v-else :width="55" :height="16" animated /></div>
          <div class="cov-row"><span class="cov-k">Índice rotación</span><b class="cov-v" v-if="!isLoading">{{
            rotationIndex
          }} v/año</b><b-skeleton v-else :width="55" :height="16" animated /></div>
        </div>
      </div>
      ...
      <div v-if="canExportReports" class="gcard mb-4">
        ...
        <div class="gc-body">
          <div class="an-export-tiles">
            <div class="an-export-tile" v-if="canSeeInventory"><i
                class="fas fa-boxes-stacked"></i><span>Inventario</span>
            </div>
            <div class="an-export-tile"><i class="fas fa-flask-vial"></i><span>Pedidos</span></div>
            <div class="an-export-tile" v-if="canSeeDevolutions"><i
                class="fas fa-rotate-left"></i><span>Devoluciones</span>
            </div>
