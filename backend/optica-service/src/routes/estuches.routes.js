...
router.get("/trash", async (req, res) => {
  try {
    const items = await Estuche.find({ isDeleted: true }).sort({ deletedAt: -1 }).lean();
    return res.json({ ok: true, data: items });
  } catch (err) {
    return res.status(500).json({ ok: false, error: err.message });
  }
...
router.get("/:id", param("id").isMongoId(), validate, async (req, res) => {
  try {
    const item = await Estuche.findById(req.params.id).lean();
...
router.post("/", bodyRules, sanitizeMiddleware(TEXT_FIELDS), validate, async (req, res) => {
  try {
    ...
    const item = await Estuche.create(fields);
    console.log(`[OPTICA][ESTUCHES] POST / → Creado SKU ${item.sku} por ${actor?.name || "Sistema"}`);
...
router.put("/:id", param("id").isMongoId(), bodyRules, sanitizeMiddleware(TEXT_FIELDS), validate, async (req, res) => {
  try {
    ...
    if (fields.sku && fields.sku !== item.sku) {
      const dup = await Estuche.findOne({ sku: fields.sku, _id: { $ne: item._id } });
...
// ── POST /estuches/:id/sale — Venta atómica ────────────────────────────────
router.post("/:id/sale", param("id").isMongoId(), body("qty").optional().isInt({ min: 1 }), validate, async (req, res) => {
  try {
    const qty = req.body.qty || 1;
    const actor = req.user;
    const result = await handleAtomicSale(Estuche, COLLECTION, req.params.id, qty, actor);
