  ...
  return a && typeof a === "object"
    ? {
        userId: isDef(a.userId)
          ...
          ? String(a._id)
          : null,
        name: isDef(a.name) ? String(a.name) : isDef(a.email) ? String(a.email) : null,
      }
    : null;
};
...
const denormNum = (s) => {
  if (s === "x") return null;
  if (String(s).startsWith("m")) return -Number(String(s).slice(1).replace("d", "."));
...
const skuPart = (s, max = 10) => {
  const raw = String(s ?? "X")
    ...
    .replace(/[^a-z0-9]+/gi, "") // solo alfanum
    .toUpperCase();
  return (raw || "X").slice(0, max);
};

// Abreviación orientada a SKU: intenta obtener las siglas/primeros caracteres
// de tokens separados por espacio/-, limitado a `len` caracteres.
const skuAbbrev = (s, len = 3) => {
  ...
  // Si hay varias palabras, tomar la primera letra de hasta 3 palabras
  if (tokens.length >= 2) {
...
const makeSheetSkuCandidate = (sheetLike) => {
  // Formato compacto: RSBO-<TIP>-<TRT>-<MAT>-<BAS>-<HEX4>
  // - TIP: tipo_matriz abreviado (3)
  // - TRT: tratamientos (iniciales hasta 3)
  // - MAT: material (3)
  // - BAS: baseKey (3)
  // - HEX4: sufijo aleatorio de 4 hex para reducir colisiones
  const tipo = skuAbbrev(sheetLike?.tipo_matriz || "X", 3);
  const trat = Array.isArray(sheetLike?.tratamientos) && sheetLike.tratamientos.length
  ...
  const rnd = crypto.randomBytes(2).toString("hex").toUpperCase(); // 4 chars
  // construir partes evitando guiones redundantes
  const parts = ["RSBO", tipo];
...
const makeUniqueSheetSku = async (sheetLike) => {
  ...
  return `RSBO-${skuPart(sheetLike?.tipo_matriz || "X", 8)}-${Date.now().toString(36).toUpperCase()}`;
};
...
const fmt2 = (n) =>
  n === null || n === undefined || Number.isNaN(Number(n)) ? "x" : Number(n).toFixed(2);
...
const ean13CheckDigit = (body12) => {
  const digits = String(body12).split("").map((d) => parseInt(d, 10) || 0);
  let sum = 0;
...
function pickRange(src, fallback) {
  ...
  const step = Math.abs(Number(isDef(r.step) ? r.step : f.step)) || Math.abs(Number(f.step)) || 0.25;
...
const rangeMinMax = (r) => {
  ...
  if (!Number.isFinite(a) || !Number.isFinite(b) || !Number.isFinite(step) || step <= 0) return null;
...
const buildTabsForTipo = (sheetOrTipo) => {
  ...
  const tipo = typeof sheetOrTipo === "string" ? sheetOrTipo : sheetOrTipo.tipo_matriz;
  if (!tipo) return [];
  ...
  switch (tipo) {
    ...
    case "SPH_ADD": {
      ...
      return [
        { id: "sph-neg", label: "SPH (-)", ranges: { sph: { start: 0, end: negEnd, step: sphR.step || 0.25 }, addCols } },
        { id: "sph-pos", label: "SPH (+)", ranges: { sph: { start: 0, end: posEnd, step: sphR.step || 0.25 }, addCols } },
      ];
    }
    ...
    case "BASE": {
      ...
      return [
        { id: "base-neg", label: "BASE (-)", ranges: { base: { start: 0, end: negEnd, step: baseR.step || 0.25 } } },
        { id: "base-pos", label: "BASE (+)", ranges: { base: { start: 0, end: posEnd, step: baseR.step || 0.25 } } },
      ];
    }
    ...
    case "BASE_ADD": {
      ...
      return [
        { id: "base-neg", label: "BASE (-) / ADD (+)", ranges: { base: { start: 0, end: negEnd, step: baseR.step || 0.25 }, addCols } },
        { id: "base-pos", label: "BASE (+) / ADD (+)", ranges: { base: { start: 0, end: posEnd, step: baseR.step || 0.25 }, addCols } },
      ];
    }
...
const maybeExtendMetaRangesFromRows = async (sheet, rows) => {
  ...
  sheet.meta.ranges = sheet.meta.ranges && typeof sheet.meta.ranges === "object" ? sheet.meta.ranges : {};

  const current =
    sheet.meta.ranges[tipo] && typeof sheet.meta.ranges[tipo] === "object" ? sheet.meta.ranges[tipo] : def;
  ...
  if (tipo === "BASE_ADD") {
    const bases = rows.map((r) => to2(r.base ?? r.base_izq ?? 0)).filter(Number.isFinite);
    const adds = rows.map((r) => to2(r.add)).filter(Number.isFinite);
...
const seedFullForSheet = async (sheet, actor) => {
  ...
  if (tipo === "BASE") {
    ...
    baseVals.forEach((base) => {
      const k = keyBase(base);
      if (!doc.cells.has(k)) {
        doc.cells.set(k, setCell({ existencias: 0, sku: makeSku(sheet._id, "BASE", { base }), codebar: null }));
      }
    });
    doc.markModified("cells");
    await doc.save();
...
const validateChunkRows = (tipo, rows) => {
  ...
  rows.forEach((row, index) => {
    ...
    if (tipo === "SPH_ADD") {
      ...
      if (!["OD", "OI"].includes(eye)) errors.push({ path: `${path}.eye`, msg: "eye debe ser OD u OI" });
  ...
  async (req, res) => {
    ...
    try {
      ...
      const sheet = await InventorySheet.create({
        nombre,
        sku: sheetSku || (await makeUniqueSheetSku({ tipo_matriz: req.body.tipo_matriz, baseKey: req.body.baseKey, material: req.body.material })),
      ...
      res.status(500).json({ ok: false, message: "Error al crear hoja", details: String(err?.message || err) });
    }
  }
);
...
router.patch(
  "/sheets/:sheetId",
  param("sheetId").isMongoId(),
  oneOf([body("nombre").optional().isString().trim().notEmpty(), body("name").optional().isString().trim().notEmpty()]),
  body("tratamientos").optional().isArray(),
  ...
  async (req, res) => {
    ...
    try {
      ...
      res.json({
        ok: true,
        data: { sheet: updated, tabs: buildTabsForTipo(updated), physicalLimits: PHYSICAL_LIMITS },
      });
    } catch (err) {
...
router.patch(
  ...
  async (req, res) => {
    ...
    try {
      ...
      res.json({ ok: true, data: { sheet, tabs: buildTabsForTipo(sheet), physicalLimits: PHYSICAL_LIMITS } });
    } catch (err) {
  ...
  async (req, res) => {
    try {
      ...
      if (sheet.tipo_matriz === "BASE") {
        ...
        if (!baseRange)
          return res.status(400).json({ ok: false, message: "Rango BASE inválido" });
      ...
      if (sheet.tipo_matriz === "SPH_ADD") {
        ...
        const addRange = clampRange(
          isDef(req.query.addMin) ? Number(req.query.addMin) : PHYSICAL_LIMITS.ADD.min,
          isDef(req.query.addMax) ? Number(req.query.addMax) : PHYSICAL_LIMITS.ADD.max,
          PHYSICAL_LIMITS.ADD
        );
        if (!sphRange) return res.status(400).json({ ok: false, message: "Rango SPH inválido" });
        if (!addRange) return res.status(400).json({ ok: false, message: "Rango ADD inválido" });
        ...
        if (doc?.cells) {
          for (const [k, cell] of doc.cells.entries()) {
            ...
            if (
              ...
              if (eyes.includes("OD"))
                rows.push({ sheet: sheet._id, tipo_matriz: "SPH_ADD", sph, add, eye: "OD", base_izq: bi, base_der: bd, existencias: cell.OD?.existencias ?? 0, sku: cell.OD?.sku || null, codebar: cell.OD?.codebar || null });
              if (eyes.includes("OI"))
                rows.push({ sheet: sheet._id, tipo_matriz: "SPH_ADD", sph, add, eye: "OI", base_izq: bi, base_der: bd, existencias: cell.OI?.existencias ?? 0, sku: cell.OI?.sku || null, codebar: cell.OI?.codebar || null });
            }
          }
        }

        rows = rows
          .sort((a, b) => (a.sph === b.sph ? (a.add === b.add ? a.eye.localeCompare(b.eye) : a.add - b.add) : a.sph - b.sph))
          .slice(0, limit);
      ...
      if (sheet.tipo_matriz === "BASE_ADD") {
        ...
        if (doc?.cells) {
          for (const [k, cell] of doc.cells.entries()) {
            ...
            if (Number.isFinite(add) && add >= addRange.min && add <= addRange.max) {
              if (eyes.includes("OD"))
                rows.push({ sheet: sheet._id, tipo_matriz: "BASE_ADD", add, eye: "OD", base_izq: bi, base_der: bd, existencias: cell.OD?.existencias ?? 0, sku: cell.OD?.sku || null, codebar: cell.OD?.codebar || null });
              if (eyes.includes("OI"))
                rows.push({ sheet: sheet._id, tipo_matriz: "BASE_ADD", add, eye: "OI", base_izq: bi, base_der: bd, existencias: cell.OI?.existencias ?? 0, sku: cell.OI?.sku || null, codebar: cell.OI?.codebar || null });
            }
          }
        }

        rows = rows
          .sort((a, b) => (a.add === b.add ? a.eye.localeCompare(b.eye) : a.add - b.add))
          .slice(0, limit);
      ...
      switch (sheet.tipo_matriz) {
        ...
        case "SPH_CYL": {
          const normalizedRows = rows.map((r) => {
            let cyl = r.cyl;
            if (Number.isFinite(Number(cyl)) && Number(cyl) > 0) cyl = -Math.abs(Number(cyl));
            return { ...r, cyl };
          });
