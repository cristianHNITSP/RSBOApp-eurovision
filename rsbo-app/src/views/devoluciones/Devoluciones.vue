<template>
  <div class="dv-root">
    ...
    <header class="dv-hero">
      ...
      <div class="dv-hero-inner">
        ...
        <div class="dv-hero-right">
          ...
          <b-button v-if="canCreateDevolution" type="is-primary" icon-left="plus" @click="openCreate">Nueva
            devolución</b-button>
          <b-button type="is-light" icon-left="arrows-rotate" :loading="loading" @click="load" />
        </div>
      </div>
    </header>
      ...
      <div class="dv-kpi-grid">
        <div class="dv-kpi-card">
          ...
          <div class="dv-kpi-inner">
            <div class="dv-kpi-ico" style="background:rgba(249,115,22,.12);color:#f97316"><i class="fas fa-clock"></i>
            </div>
            <div>
        ...
        <div class="dv-kpi-card">
          ...
          <div class="dv-kpi-inner">
            <div class="dv-kpi-ico" style="background:rgba(59,130,246,.12);color:#3b82f6"><i
        ...
        <div class="dv-kpi-card">
          ...
          <div class="dv-kpi-inner">
            <div class="dv-kpi-ico" style="background:rgba(16,185,129,.12);color:#10b981"><i
        ...
        <div class="dv-kpi-card">
          ...
          <div class="dv-kpi-inner">
            <div class="dv-kpi-ico" style="background:rgba(239,68,68,.12);color:#ef4444"><i
        ...
        <div class="dv-kpi-card">
          ...
          <div class="dv-kpi-inner">
            <div class="dv-kpi-ico" style="background:rgba(124,58,237,.12);color:#7c3aed"><i
        ...
        <div class="dv-kpi-card">
          ...
          <div class="dv-kpi-inner">
            <div class="dv-kpi-ico" style="background:rgba(6,182,212,.12);color:#06b6d4"><i
                class="fas fa-calendar-days"></i></div>
            <div>
      ...
      <div class="gcard">
        ...
        <div class="gc-head gc-head-filter">
          <div class="filter-tabs">
            <button v-for="tab in STATUS_TABS" :key="tab.value" class="ftab"
              :class="{ active: activeStatus === tab.value }" @click="setStatus(tab.value)">
              <i :class="tab.icon"></i>
              {{ tab.label }}
          ...
          <div class="search-wrap">
            ...
            <input v-model="searchQ" class="search-input" placeholder="Buscar folio, cliente, pedido..."
              @keyup.enter="load" />
        ...
        <div class="gc-body dv-list">
          ...
          <!-- Loading state -->
          <div v-if="loading" class="dv-loading-wrap">
            <b-loading :is-full-page="false" v-model="loading" :can-cancel="false" />
            <p class="has-text-grey is-size-7 mt-5">Cargando devoluciones...</p>
          </div>
              ...
              <div class="dv-card" v-for="dev in group.items" :key="dev._id">
                ...
                <div class="card-body">
                  ...
                  <div class="card-items" v-if="dev.items?.length">
                    <span class="item-chip" v-for="(it, i) in dev.items.slice(0, 4)" :key="i">
                      ...
                      <span v-if="it.restoreStock" class="item-restore" title="Reingresa stock"><i
                          class="fas fa-arrow-up-to-line"></i></span>
                    </span>
                ...
                <div class="card-actions" v-if="canManageDevolutions">
                  <template v-if="dev.status === 'pendiente'">
                    <button class="act-btn act-review" @click="quickAction(dev, 'en_revision')"
      ...
      <div class="modal-card" v-if="detailDev">
        ...
        <section class="modal-card-body">
          <div class="modal-rows">
            ...
            <div class="modal-row" v-if="detailDev.clientePhone"><span class="mrow-label">Teléfono</span><span>{{
              detailDev.clientePhone }}</span></div>
            <div class="modal-row" v-if="detailDev.orderFolio"><span class="mrow-label">Pedido vinculado</span><span>{{
              detailDev.orderFolio }}</span></div>
            <div class="modal-row"><span class="mrow-label">Motivo</span><span>{{ reasonLabel(detailDev.reason)
            }}</span>
            </div>
            <div class="modal-row" v-if="detailDev.reasonDetail"><span class="mrow-label">Detalle</span><span>{{
              detailDev.reasonDetail }}</span></div>
            <div class="modal-row" v-if="detailDev.notes"><span class="mrow-label">Notas internas</span><span>{{
              detailDev.notes }}</span></div>
            <div class="modal-row"><span class="mrow-label">Creado por</span><span>{{ detailDev.createdBy?.name || '—'
            }}</span></div>
            <div class="modal-row"><span class="mrow-label">Fecha</span><span>{{ fmtDate(detailDev.createdAt) }}</span>
            </div>
            <div class="modal-row" v-if="detailDev.processedBy?.name"><span class="mrow-label">Procesado
                por</span><span>{{
                  detailDev.processedBy.name }}</span></div>
            <div class="modal-row" v-if="detailDev.stockRestored"><span class="mrow-label">Stock</span><span
                class="stock-ok"><i class="fas fa-check"></i> Restaurado al inventario</span></div>
          </div>
          ...
          <div v-if="detailDev.items?.length" class="mt-4">
            ...
            <div class="item-row" v-for="(it, i) in detailDev.items" :key="i">
              ...
              <div class="item-row-meta">
                ...
                <span class="item-cond" :class="'cond-' + it.condition">{{ condLabel(it.condition) }}</span>
                <span v-if="it.restoreStock" class="item-restore"><i class="fas fa-arrow-up-to-line"></i> Reingresa
                  stock</span>
              </div>
            </div>
          </div>

          <div v-if="canManageDevolutions && detailDev.status !== 'procesada' && detailDev.status !== 'rechazada'"
            class="mt-4">
        ...
        <footer class="modal-card-foot" style="justify-content:flex-end;gap:0.5rem;flex-wrap:wrap"
          ...
          <b-button v-if="detailDev.status === 'pendiente'" type="is-info" icon-left="magnifying-glass"
            @click="applyAction(detailDev, 'en_revision')">En revisión</b-button>
          <b-button v-if="detailDev.status === 'en_revision' || detailDev.status === 'pendiente'" type="is-success"
            icon-left="check" @click="applyAction(detailDev, 'aprobada')">Aprobar</b-button>
          <b-button v-if="detailDev.status === 'aprobada'" type="is-primary" icon-left="box-archive"
            @click="applyAction(detailDev, 'procesada')">Procesar</b-button>
          <b-button v-if="['pendiente', 'en_revision', 'aprobada'].includes(detailDev.status)" type="is-danger"
            icon-left="xmark" @click="applyAction(detailDev, 'rechazada')">Rechazar</b-button>
        </footer>
    ...
    <b-modal v-model="showCreate" has-modal-card trap-focus :destroy-on-hide="true" animation="zoom-in">
      <div class="modal-card dv-create-card">
        ...
        <section class="modal-card-body">
          ...
          <div class="mb-4" v-if="form.items.length === 0">
            <TransactionSearch label="1. Buscar Venta o Pedido para Devolución" @select="onTransactionSelected" />

            <TransactionItemPicker v-if="selectedTransaction" class="mt-3"
              :transaction-folio="selectedTransaction.folio" :items="selectedTransaction.items" @add="onItemAdded" />
          </div>
          ...
          <div class="form-grid">
            ...
            <b-field label="Folio de Venta / Pedido (opcional)">
              <b-input v-model="form.orderFolio" placeholder="Ej: VNT-1234 o LAB-5678" />
            </b-field>
          ...
          <b-field label="Descripción del problema" class="mt-3">
            <b-input v-model="form.reasonDetail" type="textarea" rows="2"
              placeholder="Describe el problema en detalle..." />
          </b-field>
          <b-field label="Notas internas">
            <b-input v-model="form.notes" type="textarea" rows="2"
          ...
          <div class="items-section mt-4">
            <div class="msec-title">
              ...
              <div v-if="form.items.length > 0">
                <b-button size="is-small" type="is-light" icon-left="plus" @click="addItem">Manual</b-button>
                <b-button v-if="selectedTransaction" size="is-small" type="is-primary is-light" class="ml-2"
                  icon-left="list-check" @click="showPicker = true">Desde Venta</b-button>
              </div>
              <b-button v-else size="is-small" type="is-light" icon-left="plus" @click="addItem">Agregar
                Manual</b-button>
            </div>

            <b-modal v-model="showPicker" v-if="selectedTransaction" has-modal-card trap-focus>
              <div class="modal-card" style="width: 500px">
                <header class="modal-card-head">
                  <p class="modal-card-title">Seleccionar ítems de {{ selectedTransaction.folio }}</p>
                  <button class="delete" @click="showPicker = false" />
                </header>
                <section class="modal-card-body">
                  <TransactionItemPicker :transaction-folio="selectedTransaction.folio"
                    :items="selectedTransaction.items" @add="onItemAdded" />
                </section>
              </div>
            </b-modal>
            ...
            <div class="form-restore-note" v-if="form.items.some(i => i.restoreStock)">
              ...
              Los ítems marcados con <i class="fas fa-arrow-up-to-line"></i> reingresarán stock al inventario cuando se
    ...
    <DevolutionEditModal v-model="editOpen" :devolution="editTarget" @saved="onEditSaved" />
...
function onTransactionSelected(tx) {
  ...
  form.value.cliente = tx.cliente;
  form.value.orderFolio = tx.folio;
  labToast.info(`Venta ${tx.folio} seleccionada. Ahora elige los productos.`);
}

function onItemAdded(item) {
  // Evitar duplicados si ya está en la lista
  const exists = form.value.items.find(i => i.matrixKey === item.matrixKey && i.eye === item.eye && i.codebar === item.codebar);
  if (exists) {
    labToast.warning("Este producto ya está en la lista");
    return;
  }
  ...
  labToast.success(`Agregado: ${item.title}`);
  showPicker.value = false;
}
function addItem() {
...
.dv-card {
  display: flex;
  background: var(--surface-solid, var(--g-bg));
  border: 1px solid var(--g-border);
  border-radius: 12px;
