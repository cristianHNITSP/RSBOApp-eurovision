<!-- ============================================================
  ...
  ============================================================ -->
<template>
  <div class="grid-page" :class="{ 'is-fullscreen': isFullscreen, 'ag-grid-fullscreen-container': isFullscreen, 'is-hibernating': isHibernating }"
    ref="gridPageRef">
...
function markChanged(data, field, newValue, _oldValue) {
  ...
  pendingChanges.value.set(key, { sph: s, add: a, eye: e, base_izq: to2(data.base_izq ?? 0), base_der: to2(data.base_der ?? 0), existencias: newVal, baseline });
  dirty.value = true;
  ...
  return [
    {
      ...
      children: [{
        ...
        valueFormatter: (p) => { if ((p.data?.__loading || !p.data)) return ""; const v = Number(p.value); return Number.isFinite(v) ? v.toFixed(2) : p.value ?? ""; },
...
async function _refreshCachedRows() {
  ...
  try {
    ...
    pageSphs.forEach(sph => {
      ...
      items.filter(i => i.sph === sph).forEach(i => {
        const field = `add_${norm(i.add)}_${i.eye}`;
        const pk = `${to2(sph)}|${to2(i.add)}|${i.eye}`;
