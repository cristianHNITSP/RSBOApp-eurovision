// src/routes/contactlenses.routes.js
...
const schemaHas = (path) => {
  try {
    return !!ContactLensesSheet?.schema?.path?.(path);
  } catch {
    return false;
  }
};
...
if (DEBUG_INVENTORY) {
  console.log("[CL][BOOT] ContactLensesSheet schema paths present?", {
...
router.use((req, res, next) => {
  ...
  console.log("[CL][REQ]", req.method, req.originalUrl, {
...
const handleValidation = (req, res, next) => {
  ...
  if (!errors.isEmpty()) {
    if (DEBUG_INVENTORY) {
      console.error("[CL][VALIDATION] FAIL", req.method, req.originalUrl, {
        purchase: pickPurchase(req.body),
        errors: errors.array()
      });
    }
...
router.get("/", async (req, res) => {
  try {
    ...
    const sheets = await ContactLensesSheet.find(query).sort({ updatedAt: -1, createdAt: -1 });
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      if (DEBUG_INVENTORY) {
        console.log("[CL][POST /sheets] schema check:", {
          numFactura: schemaHas("numFactura"),
          loteProducto: schemaHas("loteProducto"),
          fechaCompra: schemaHas("fechaCompra"),
          fechaCaducidad: schemaHas("fechaCaducidad")
        });
        console.log("[CL][POST /sheets] incoming purchase:", pickPurchase(req.body));
      }
      ...
      const skuFinal = sheetSku || (await makeUniqueSheetSku(ContactLensesSheet, sheetLikeForSku));
      ...
      const sheet = await ContactLensesSheet.create(payloadCreate);
      ...
      console.error("POST /contactlenses/sheets error:", err);
      res.status(500).json({ ok: false, message: "Error al crear hoja de lente de contacto", details: String(err?.message || err) });
    }
  }
);
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      if (!sheet.sku) {
        try {
          await ensureSheetSku(ContactLensesSheet, sheet);
        } catch (e) {
      ...
      console.error("GET /contactlenses/sheets/:sheetId error:", err);
...
router.get("/sheets/by-sku/:sku", async (req, res) => {
  try {
    ...
    const sheet = await ContactLensesSheet.findOne({ sku, isDeleted: { $ne: true } });
...
router.patch(
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (hasFechaCompra && !hasFechaCad) sheet.fechaCaducidad = null;
      ...
      if (!preserveSku) {
        if (skuManualProvided) {
          ...
          sheet.sku = await makeUniqueSheetSku(ContactLensesSheet, sheetLikeForSku, sheet._id);
        } else if (!sheet.sku) {
          await ensureSheetSku(ContactLensesSheet, sheet);
        }
      }
      ...
      console.error("PATCH /contactlenses/sheets/:sheetId error:", err);
...
router.patch(
  ...
  async (req, res) => {
    ...
    try {
      ...
      console.error("PATCH /contactlenses/sheets/:sheetId/trash error:", err);
  ...
  async (req, res) => {
    try {
      ...
      if (sheet.tipo_matriz === "SPH_ADD") {
        ...
        const eyes = String(req.query.eyes || "OD,OI").split(",").map((e) => e.trim().toUpperCase());
      ...
      console.error("GET /contactlenses/sheets/:sheetId/items error:", err);
