<template>
  <div class="an-tab-single-col is-flex-grow-1">
    <div class="gcard mb-5">
      <div class="gc-head">
        <div class="gc-ico" style="background:var(--c-info-alpha);color:var(--c-info)">
      ...
      <div class="gc-body">
        <div class="an-stat-grid">
          ...
          <div class="an-stat-cell">
            <div class="asc-ico" style="background:var(--c-info-alpha);color:var(--c-info)">
              <i class="fas fa-rotate"></i>
            </div>
            <div v-if="!isLoading" class="asc-val"
              :style="{ color: (s?.corrections7d ?? 0) > 5 ? 'var(--c-warning)' : 'var(--c-success)' }">
              {{ s?.corrections7d ?? 0 }}
            </div>
          ...
          <div class="an-stat-cell">
            ...
            <div v-if="!isLoading" class="asc-val" style="color:var(--c-info)">
              {{ s?.scansToday ?? 0 }}
            </div>
            <b-skeleton v-else :width="60" :height="28" animated />
        ...
        <div v-if="!isLoading" class="an-quality-bar mt-4">
          <div class="aqb-head">
            ...
            <span class="aqb-val"
              :style="{ color: correctionRate > 10 ? 'var(--c-danger)' : correctionRate > 5 ? 'var(--c-warning)' : 'var(--c-success)' }">
              {{ correctionRate }}%
            </span>
          </div>
