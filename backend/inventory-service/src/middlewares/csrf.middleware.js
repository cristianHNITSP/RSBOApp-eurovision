...
function csrfProtection(req, res, next) {
  ...
  if (!headerToken || !cookieToken) {
    return res.status(403).json({ ok: false, error: "CSRF_MISSING", message: "Token CSRF no proporcionado" });
  }

  const a = Buffer.from(String(headerToken));
  const b = Buffer.from(String(cookieToken));
  if (a.length !== b.length || !crypto.timingSafeEqual(a, b)) {
    return res.status(403).json({ ok: false, error: "CSRF_INVALID", message: "Token CSRF inválido" });
  }
